...
Run from phase-3/ directory:
    ANTHROPIC_API_KEY=sk-... python -m state_graph.faculty_hiring.demo_faculty_hiring
...
def run_demo():
    ...
    for c in result.get("candidates", []):
        print(f"      - {c['name']}: score={c.get('score')}, parse_status={c.get('parse_status')}")
    ...
    print(f"      candidate_ids=[<id>], decided_by='dept_head', reason='...')")
